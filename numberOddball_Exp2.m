function numeroOddball_main_Exp2(foldersToUse,plotSplit,chanToCompare)
    if nargin == 0
        foldersToUse = [];
        ...
        for fPair = 1 %was 1:3 before
            for rcType = 1:2
                ...
                for t=1:6 % number of tTests
                    results = tSquaredFourierCoefs(xyData(:,:,contrastOrder(t,1)),xyData(:,:,contrastOrder(t,2)));
    ...
    for r = 1:yFigs 
        ...
        for t = 1:2
            ...
            for c=1:3
                ...
                if plotSNR
                    curRange = snrVals(:,:,curOrder,1,plotSplit+1);
                    %valSet = snrVals(curIdx,r,c,f,plotSplit+1);
                    ampH(c)=plot(1:numFreqs,snrVals(curIdx,r,curOrder(c),1,plotSplit+1),'-','LineWidth',lWidth,'Color',subColors(c,:));
                else
                    curRange = ampVals(:,:,curOrder(c),1,plotSplit+1);
                    %valSet = ampVals(:,r,c,f,plotSplit+1);
                    ampH(c)=plot(1:numFreqs,ampVals(curIdx,r,curOrder(c),1,plotSplit+1),'-','LineWidth',lWidth,'Color',subColors(c,:));
                    %plot(1:2,noiseVals(curIdx,r,c,f,plotSplit+1),'sq','Color',subColors(c,:),'MarkerSize',5);
                    errorbar(1:numFreqs,ampVals(curIdx,r,curOrder(c),1,plotSplit+1),errLB(curIdx,r,curOrder(c),1,plotSplit+1),errUB(curIdx,r,curOrder(c),1,plotSplit+1),'Color',subColors(c,:),'LineWidth',lWidth);
                end
                yMax = ceil(max(curRange(:)));
                zeroSig = tVs0Pval(curIdx,r,curOrder(c),1,plotSplit+1)<0.05;
                if any(zeroSig)
                    if c==1
                        arrayfun(@(x) ...
            ...
            curSig = tPval(curIdx,r,1,plotSplit+1,:,f)<0.05;
            curSig = curSig+(tPval(curIdx,r,1,plotSplit+1,:,f)<0.005);
